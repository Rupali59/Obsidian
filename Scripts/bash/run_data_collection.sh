...
# Layout: Scripts/bash/this.sh → vault is ../..
SCRIPTS_DIR="$(cd "$(dirname "$0")/.." && pwd)"
OBSIDIAN_PATH="$(cd "$(dirname "$0")/../.." && pwd)"
CONFIG_PATH="$SCRIPTS_DIR/config/unified_data_config.json"
...
# Preflight: Search API batched by repo:… OR … (committer-date + created) before hitting every repo.
# Includes AI/bot commits (committer-date, not author-only). Personal accounts: use repos (default).
# GITHUB_PREFLIGHT=author (cheap, misses AI commits). GITHUB_PREFLIGHT=namespace (org accounts). off = disable.
export GITHUB_PREFLIGHT="${GITHUB_PREFLIGHT:-repos}"
...
def process_date(target_date):
    ...
    try:
        ...
        ], cwd="$SCRIPTS_DIR/python", check=False, capture_output=True, text=True)
...
if [ $# -eq 0 ]; then
    ...
    (cd "$SCRIPTS_DIR/python" && python3 -m data_collectors.main --config "$CONFIG_PATH" --today)
    
elif [ "$1" = "yesterday" ]; then
    echo "📅 Processing yesterday"
    YESTERDAY=$(date -v-1d '+%Y-%m-%d' 2>/dev/null || date -d 'yesterday' '+%Y-%m-%d')
    (cd "$SCRIPTS_DIR/python" && python3 -m data_collectors.main --config "$CONFIG_PATH" --date "$YESTERDAY")
    ...
    (cd "$SCRIPTS_DIR/python" && python3 -m data_collectors.main --config "$CONFIG_PATH" --date "$1")
fi
